...
generate_standard_format_files() {
  ...
  "$VENV_BIN_DIRECTORY"/poetry run python3 "$EVS_OPS_HOME"/src/terminology_converter/converter/umls_sem_net.py -d "$INPUT_DIRECTORY/SRDEF" -i "$INPUT_DIRECTORY/SRSTRE1" -s "$INPUT_DIRECTORY/SRSTR" -o "$OUTPUT_DIRECTORY" 2>&1
}
